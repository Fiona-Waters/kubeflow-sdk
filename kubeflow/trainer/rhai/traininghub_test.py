...
def test_traininghub_uses_runtime_command():
    """Test that LoRA uses torchrun since it doesn't manage its own distributed training."""
    print("Executing test: TrainingHub LoRA uses torchrun command")
    ...
    # Create TrainingHub trainer with LoRA (does NOT manage its own distributed)
    trainer = TrainingHubTrainer(
    ...
    # Verify command uses torchrun for LoRA
    command_str = " ".join(trainer_crd.command)
    assert "torchrun" in command_str, f"LoRA should use torchrun: {command_str}"

    print("test execution complete")


def test_traininghub_multinode_with_torch_runtime():
    """Test multi-node: SFT uses python (manages own distributed), LoRA uses torchrun."""
    print("Executing test: Multi-node TrainingHub with torch runtime")

    from kubeflow.trainer.rhai.traininghub import get_trainer_cr_from_training_hub_trainer
    from kubeflow.trainer.types import types

    # --- LoRA multi-node: should use torchrun ---
    runtime = types.Runtime(
        name="torch-runtime",
        trainer=types.RuntimeTrainer(
            trainer_type=types.TrainerType.CUSTOM_TRAINER,
    ...
    command_str = " ".join(trainer_crd.command)
    assert "torchrun" in command_str, f"LoRA multi-node MUST use torchrun: {command_str}"

    # --- SFT multi-node: should use python (manages own distributed) ---
    runtime2 = types.Runtime(
        name="torch-runtime",
        trainer=types.RuntimeTrainer(
            trainer_type=types.TrainerType.CUSTOM_TRAINER,
            framework="pytorch",
    ...
    runtime2.trainer.set_command(constants.TORCH_COMMAND)

    trainer2 = TrainingHubTrainer(
        algorithm=TrainingHubAlgorithms.SFT,
        func_args={
            "data_path": "/data/train.jsonl",
            "ckpt_output_dir": "/tmp/checkpoints",
    ...
    trainer_crd2 = get_trainer_cr_from_training_hub_trainer(runtime2, trainer2)
    command_str2 = " ".join(trainer_crd2.command)
    assert "python" in command_str2, (
        f"SFT multi-node should use python (manages own distributed): {command_str2}"
    )
    assert "torchrun" not in command_str2, (
        f"SFT multi-node should NOT use torchrun (avoids nested torchrun): {command_str2}"
    )
...
def test_traininghub_command_matches_customtrainer_pattern():
    ...
    trainer = TrainingHubTrainer(
        ...
        algorithm=TrainingHubAlgorithms.SFT,  # SFT manages own distributed
    )
    ...
    # SFT manages own distributed, so should use python not torchrun
    assert "python" in command_str, "SFT should use python entrypoint"
    assert "torchrun" not in command_str, "SFT should NOT use torchrun"

    print("test execution complete")


def test_traininghub_sft_single_node_uses_python():
    """Test that SFT uses python entrypoint (manages own distributed training)."""
    print("Executing test: SFT single-node uses python")
    ...
    # Create single-node SFT trainer
    trainer = TrainingHubTrainer(
        algorithm=TrainingHubAlgorithms.SFT,
        func_args={"data_path": "/data/train.jsonl", "ckpt_output_dir": "/tmp/checkpoints"},
    )
    ...
    # SFT manages its own distributed training, so should use python (not torchrun)
    command_str = " ".join(trainer_crd.command)
    assert "python" in command_str, f"SFT should use python entrypoint: {command_str}"
    assert "torchrun" not in command_str, (
        f"SFT should NOT use torchrun (avoids nested torchrun): {command_str}"
    )
...
def test_sft_algorithm_uses_python_entrypoint():
    """Test that SFT algorithm uses python entrypoint (manages own distributed training)."""
    print("Executing test: SFT algorithm uses python entrypoint")
    ...
    assert "python" in command_str, f"SFT should use python: {command_str}"
    assert "torchrun" not in command_str, f"SFT should NOT use torchrun: {command_str}"
...
def test_lora_algorithm_uses_torchrun_entrypoint():
    """Test that LoRA algorithm uses torchrun entrypoint (needs external torchrun)."""
    print("Executing test: LoRA algorithm uses torchrun entrypoint")
